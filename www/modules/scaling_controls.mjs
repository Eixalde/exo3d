...
class ScalingControls {
	...
	constructor({ planets, star, cameras }, scene) {
		...
		const fakeFirstPlanetTrajectory = planets[0].trajectory
    ...
    planet. */
        ...
        increases/decreases the scaling when that timeout is reached. */
              ...
              shorter to apply the step to each component individually. */
				...
				document.querySelector("#starScale").innerHTML =
					`${CHANGE_STAR_FACTOR.toFixed(0)}:1`
				document.querySelector("#planetsScale").innerHTML =
					`${CHANGE_PLANET_FACTOR.toFixed(0)}:1`
