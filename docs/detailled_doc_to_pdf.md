...
# Detailled documentation for the Exo3D project
...
Instead what we choose to do is rely on a much precise and malleable system : the Babylon Animation. This object includes all sorts of tools that fit almost everything we want by default : precision with a low amount of data, continuous movement and easy speed control for all objects. Here is how it works : give any number of keys to an animation (a key is comprised of a frame and its associated value : position in our case ; see figure \ref{anim}) and launch the animation. Babylon will automatically interpolate between those points and the object will move smoothly - without teleporting ! - between each of these points, looping infinitely. We don't have to do anything else, not even manage the time flow (see figure \ref{loop}).
...
There used to be a transition system, which made the immersion sharply better, but it was totally incompatible with the previously mentionned parenting system. The type of camera we use for the system and the planet (called ArcRotateCamera) needs a specific position for its target. It isn't impossible to use the same trick as we did above, by including the camera in the plane of the highest parent. But this means we are literally changing planes when transitionning from an object to another, which brings in all the bad calculations we wanted to avoid. This is an absolute calamity to handle and I never want to hear the concept of transition between planes, ever.
...
- the camera zooms out if its current distance is not enough to be outside of the object
