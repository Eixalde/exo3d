...
Instead what we choose to do is rely on a much precise and malleable system : the Babylon Animation. This object includes all sorts of tools that fit almost everything we want by default : precision with a low amount of data, continuous movement and easy speed control for all objects. Here is how it works : give any number of keys to an animation (a key is comprised of a frame and its associated value : position in our case) and launch the animation. Babylon will automatically interpolate between those points and the object will move smoothly - without teleporting ! - between each of these points, looping infinitely. We don't have to do anything else, not even manage the time flow.
...
| Scaling up two planets of different sizes |
| ----------------------------------------- |
